...
export const useTicketStore = defineStore("ticket", {
  ...
  actions: {
      ...
      try {
        ...
        for (const item of listaParaProcesar) {
          ...
          if (item.tipoEvento === 2) {
            ...
            const esRedundante = listaParaProcesar.some(
              (otro) =>
    ...
    async addComment(texto: string) {
      ...
      const activyStore = useActivityStore();
      activyStore.refreshMine();
      activyStore.refreshGlobal();
      toast.success("Comentario agregado");
    },
// ticketModalStore.ts
async assignManager(gestorId: string, ticketUuid: string) {
  try {
    await ticketService.assignManager(ticketUuid, gestorId);

    await ticketService.changeStatus(ticketUuid, 1, null);
    await this.fetchTicket(ticketUuid);
